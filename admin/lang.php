<?php
// ========================================================
// BAKE'N BREW - System Translation Dictionary (id & en)
// ========================================================

$lang_code = (isset($admin_data['lang']) && $admin_data['lang'] === 'id') ? 'id' : 'en';

$dict = [
    'id' => [
        // Sidebar & Navbar
        'nav_dashboard' => 'Dashboard',
        'nav_menu' => 'Kelola Menu',
        'nav_orders' => 'Kelola Pesanan',
        'halo' => 'Halo',
        'profile' => 'Lihat Profil',
        'logout' => 'Log Out',
        'administrator' => 'Administrator',
        'notif_title' => 'Notifikasi',
        'mark_all_read' => 'Tandai Semua Dibaca',
        'view_all_notif' => 'Lihat Semua Notifikasi',
        'no_notif' => 'Tidak ada notifikasi.',
        
        // General Messages
        'db_offline_title' => 'Koneksi Database Offline',
        'db_offline_msg' => 'Peringatan: Koneksi server database terputus. Anda saat ini melihat data statis (Mode Offline).',
        'confirm_logout' => 'Apakah Anda yakin ingin keluar?',
        
        // Dashboard
        'dash_title' => 'Dashboard',
        'stat_total_menu' => 'Total Menu',
        'stat_total_order' => 'Total Pesanan',
        'stat_store_status' => 'Status Operasional Toko',
        'store_open' => 'Toko Buka',
        'store_closed' => 'Toko Tutup',
        'status_open' => 'BUKA',
        'status_closed' => 'TUTUP',
        'menu_composition' => 'Komposisi Menu',
        'view_all' => 'Lihat Semua',
        'chart_title' => 'Kategori Terpopuler',
        'recent_orders_title' => 'Pesanan Terbaru',
        'recent_orders_sub' => '5 pesanan terakhir pelanggan',
        'recent_orders_empty' => 'Belum ada pesanan masuk.',
        'col_id' => 'ID',
        'col_customer' => 'Pelanggan',
        'col_menu' => 'Menu',
        'col_qty' => 'Jumlah',
        'col_status' => 'Status',
        'status_pending' => 'Pending',
        'status_completed' => 'Selesai',
        
        // Kelola Menu
        'menu_title' => 'Kelola Menu',
        'menu_list' => 'Daftar Menu',
        'btn_add_menu' => 'Tambah Menu',
        'btn_add_menu_new' => 'Tambah Menu Baru',
        'lbl_product_photo' => 'Foto Produk',
        'lbl_menu_name' => 'Nama Menu',
        'lbl_price' => 'Harga (Rupiah)',
        'lbl_category' => 'Kategori Menu',
        'lbl_photo_hint_add' => 'Foto Produk *',
        'lbl_photo_hint_edit' => 'Foto Produk (Kosongkan jika tidak ingin diubah)',
        'lbl_description' => 'Deskripsi Menu',
        'lbl_badges' => 'Lencana',
        'lbl_best_seller' => 'Best Seller',
        'lbl_new' => 'New',
        'btn_cancel' => 'Batal',
        'btn_batal' => 'Batal',
        'msg_menu_added' => 'Menu \'%s\' berhasil ditambahkan!',
        'msg_menu_deleted' => 'Menu \'%s\' berhasil dihapus beserta gambarnya.',
        'msg_menu_updated' => 'Menu \'%s\' berhasil diperbarui!',
        'msg_order_completed' => 'Pesanan #%s berhasil ditandai selesai.',
        'msg_order_deleted' => 'Log pesanan #%s berhasil dihapus.',
        'confirm_delete_menu_item' => 'Apakah Anda yakin ingin menghapus menu \'%s\'?',
        'confirm_delete_order_item' => 'Apakah Anda yakin ingin menghapus log pesanan #%s?',
        'lbl_description_placeholder' => 'Jelaskan detail menu, rasa, dan keunggulan produk...',
        'lbl_menu_name_placeholder' => 'Contoh: Matcha Cheese Croissant',
        'lbl_price_placeholder' => 'Contoh: 28000',
        'filter_all_cat' => '— Semua Kategori —',
        'filter_btn' => 'Filter',
        'search_placeholder_menu' => 'Cari nama menu...',
        'tbl_photo' => 'Foto',
        'tbl_menu_name' => 'Nama Menu',
        'tbl_category' => 'Kategori',
        'tbl_price' => 'Harga',
        'tbl_badge' => 'Lencana',
        'tbl_actions' => 'Aksi',
        'btn_edit' => 'Edit',
        'btn_delete' => 'Hapus',
        'empty_menu' => 'Belum ada menu terdaftar.',
        'confirm_delete_menu' => 'Apakah Anda yakin ingin menghapus menu ini?',
        
        // Kelola Pesanan
        'orders_title' => 'Kelola Pesanan',
        'orders_list' => 'Log Pesanan Pelanggan',
        'orders_total' => 'Total',
        'search_placeholder_orders' => 'Cari nama, email, atau menu...',
        'filter_all_status' => '— Semua Status —',
        'tbl_notes' => 'Catatan',
        'tbl_order_time' => 'Waktu Pemesanan',
        'btn_complete' => 'Selesai',
        'empty_orders' => 'Tidak ada data pesanan yang cocok.',
        'confirm_delete_order' => 'Apakah Anda yakin ingin menghapus log pesanan ini?',
        
        // Profil
        'profile_title' => 'Profil & Pengaturan',
        'tab_profile' => 'Profil Akun',
        'tab_security' => 'Keamanan & Preferensi',
        'lbl_fullname' => 'Nama Lengkap',
        'lbl_email' => 'Email',
        'lbl_email_hint' => '(Read-only untuk keamanan)',
        'lbl_phone' => 'Nomor Telepon/WhatsApp',
        'lbl_role' => 'Role Hak Akses',
        'btn_save_changes' => 'Simpan Perubahan',
        'lbl_current_pass' => 'Password Saat Ini',
        'lbl_new_pass' => 'Password Baru',
        'lbl_confirm_pass' => 'Konfirmasi Password Baru',
        'btn_update_pass' => 'Perbarui Password',
        'lbl_sys_prefs' => 'Preferensi Sistem',
        'lbl_enable_sound' => 'Aktifkan Suara Notifikasi',
        'lbl_default_lang' => 'Bahasa Default',
        'btn_save_prefs' => 'Simpan Preferensi',
        'btn_select_photo' => 'Pilih Foto Baru',
        'btn_remove_photo' => 'Hapus Foto',
        'avatar_hint' => 'Mendukung format JPG, PNG. Maksimal 2MB.',
        'personal_data' => 'Data Diri',
        'change_password' => 'Ubah Password',
        'confirm_delete_avatar' => 'Apakah Anda yakin ingin menghapus foto profil?',
        'err_offline_post' => 'Penyimpanan data tidak diizinkan dalam Mode Offline.',
        'err_fullname_empty' => 'Nama Lengkap tidak boleh kosong.',
        'msg_profile_success' => 'Informasi profil berhasil diperbarui.',
        'err_profile_fail' => 'Gagal memperbarui profil: ',
        'msg_avatar_removed' => 'Foto profil berhasil dihapus.',
        'err_avatar_remove_fail' => 'Gagal menghapus foto profil: ',
        'err_avatar_invalid_ext' => 'Format file tidak didukung. Harap unggah file .jpg, .jpeg, atau .png.',
        'err_avatar_too_large' => 'Ukuran file terlalu besar. Maksimal ukuran file adalah 2MB.',
        'msg_avatar_uploaded' => 'Foto profil berhasil diunggah.',
        'err_avatar_db_fail' => 'Gagal menyimpan ke database: ',
        'err_avatar_upload_fail' => 'Gagal mengunggah file ke server.',
        'err_avatar_select' => 'Silakan pilih file gambar untuk diunggah.',
        'err_pass_required' => 'Semua field password wajib diisi.',
        'err_pass_mismatch' => 'Konfirmasi password baru tidak cocok.',
        'err_pass_length' => 'Password baru minimal harus 6 karakter.',
        'msg_pass_success' => 'Password berhasil diubah.',
        'err_pass_fail' => 'Gagal mengubah password: ',
        'err_pass_invalid' => 'Password saat ini tidak valid.',
        'msg_prefs_success' => 'Preferensi sistem berhasil disimpan.',
        'err_prefs_fail' => 'Gagal memperbarui preferensi: ',
        
        // Notifikasi Archive
        'notif_archive_title' => 'Pusat Notifikasi',
        'notif_all' => 'Semua Notifikasi',
        'btn_clear_all' => 'Hapus Semua',
        'lbl_view_detail' => 'Lihat Detail',
        'empty_notif_archive' => 'Belum ada notifikasi yang diterima.',
        'confirm_clear_all' => 'Apakah Anda yakin ingin menghapus semua riwayat notifikasi?'
    ],
    'en' => [
        // Sidebar & Navbar
        'nav_dashboard' => 'Dashboard',
        'nav_menu' => 'Manage Menu',
        'nav_orders' => 'Manage Orders',
        'halo' => 'Hello',
        'profile' => 'View Profile',
        'logout' => 'Log Out',
        'administrator' => 'Administrator',
        'notif_title' => 'Notifications',
        'mark_all_read' => 'Mark All Read',
        'view_all_notif' => 'View All Notifications',
        'no_notif' => 'No notifications available.',
        
        // General Messages
        'db_offline_title' => 'Database Connection Offline',
        'db_offline_msg' => 'Warning: Database server connection lost. You are currently viewing static data (Offline Mode).',
        'confirm_logout' => 'Are you sure you want to log out?',
        
        // Dashboard
        'dash_title' => 'Dashboard',
        'stat_total_menu' => 'Total Menus',
        'stat_total_order' => 'Total Orders',
        'stat_store_status' => 'Store Operational Status',
        'store_open' => 'Store Open',
        'store_closed' => 'Store Closed',
        'status_open' => 'OPEN',
        'status_closed' => 'CLOSED',
        'menu_composition' => 'Menu Composition',
        'view_all' => 'View All',
        'chart_title' => 'Most Popular Categories',
        'recent_orders_title' => 'Recent Orders',
        'recent_orders_sub' => 'Last 5 customer orders',
        'recent_orders_empty' => 'No orders placed yet.',
        'col_id' => 'ID',
        'col_customer' => 'Customer',
        'col_menu' => 'Menu',
        'col_qty' => 'Quantity',
        'col_status' => 'Status',
        'status_pending' => 'Pending',
        'status_completed' => 'Completed',
        
        // Kelola Menu
        'menu_title' => 'Manage Menu',
        'menu_list' => 'Menu List',
        'btn_add_menu' => 'Add Menu',
        'btn_add_menu_new' => 'Add New Menu',
        'lbl_product_photo' => 'Product Photo',
        'lbl_menu_name' => 'Menu Name',
        'lbl_price' => 'Price (IDR)',
        'lbl_category' => 'Menu Category',
        'lbl_photo_hint_add' => 'Product Photo *',
        'lbl_photo_hint_edit' => 'Product Photo (Leave empty if not changing)',
        'lbl_description' => 'Menu Description',
        'lbl_badges' => 'Badges',
        'lbl_best_seller' => 'Best Seller',
        'lbl_new' => 'New',
        'btn_cancel' => 'Cancel',
        'btn_batal' => 'Cancel',
        'msg_menu_added' => 'Menu \'%s\' added successfully!',
        'msg_menu_deleted' => 'Menu \'%s\' deleted successfully along with its image.',
        'msg_menu_updated' => 'Menu \'%s\' updated successfully!',
        'msg_order_completed' => 'Order #%s marked as completed successfully.',
        'msg_order_deleted' => 'Order log #%s deleted successfully.',
        'confirm_delete_menu_item' => 'Are you sure you want to delete menu \'%s\'?',
        'confirm_delete_order_item' => 'Are you sure you want to delete order log #%s?',
        'lbl_description_placeholder' => 'Describe menu details, taste, and product specialties...',
        'lbl_menu_name_placeholder' => 'Example: Matcha Cheese Croissant',
        'lbl_price_placeholder' => 'Example: 28000',
        'filter_all_cat' => '— All Categories —',
        'filter_btn' => 'Filter',
        'search_placeholder_menu' => 'Search menu name...',
        'tbl_photo' => 'Photo',
        'tbl_menu_name' => 'Menu Name',
        'tbl_category' => 'Category',
        'tbl_price' => 'Price',
        'tbl_badge' => 'Badge',
        'tbl_actions' => 'Actions',
        'btn_edit' => 'Edit',
        'btn_delete' => 'Delete',
        'empty_menu' => 'No menus registered yet.',
        'confirm_delete_menu' => 'Are you sure you want to delete this menu?',
        
        // Kelola Pesanan
        'orders_title' => 'Manage Orders',
        'orders_list' => 'Customer Orders Log',
        'orders_total' => 'Total',
        'search_placeholder_orders' => 'Search name, email, or menu...',
        'filter_all_status' => '— All Statuses —',
        'tbl_notes' => 'Notes',
        'tbl_order_time' => 'Order Time',
        'btn_complete' => 'Complete',
        'empty_orders' => 'No matching orders found.',
        'confirm_delete_order' => 'Are you sure you want to delete this order log?',
        
        // Profil
        'profile_title' => 'Profile & Settings',
        'tab_profile' => 'Account Profile',
        'tab_security' => 'Security & Preferences',
        'lbl_fullname' => 'Full Name',
        'lbl_email' => 'Email',
        'lbl_email_hint' => '(Read-only for security)',
        'lbl_phone' => 'Phone/WhatsApp Number',
        'lbl_role' => 'Access Role',
        'btn_save_changes' => 'Save Changes',
        'lbl_current_pass' => 'Current Password',
        'lbl_new_pass' => 'New Password',
        'lbl_confirm_pass' => 'Confirm New Password',
        'btn_update_pass' => 'Update Password',
        'lbl_sys_prefs' => 'System Preferences',
        'lbl_enable_sound' => 'Enable Notification Sound',
        'lbl_default_lang' => 'Default Language',
        'btn_save_prefs' => 'Save Preferences',
        'btn_select_photo' => 'Choose New Photo',
        'btn_remove_photo' => 'Remove Photo',
        'avatar_hint' => 'Supports JPG, PNG. Maximum 2MB.',
        'personal_data' => 'Personal Data',
        'change_password' => 'Change Password',
        'confirm_delete_avatar' => 'Are you sure you want to delete your profile picture?',
        'err_offline_post' => 'Data saving is not allowed in Offline Mode.',
        'err_fullname_empty' => 'Full Name cannot be empty.',
        'msg_profile_success' => 'Profile information updated successfully.',
        'err_profile_fail' => 'Failed to update profile: ',
        'msg_avatar_removed' => 'Profile picture deleted successfully.',
        'err_avatar_remove_fail' => 'Failed to delete profile picture: ',
        'err_avatar_invalid_ext' => 'Unsupported file format. Please upload .jpg, .jpeg, or .png.',
        'err_avatar_too_large' => 'File size is too large. Maximum size is 2MB.',
        'msg_avatar_uploaded' => 'Profile picture uploaded successfully.',
        'err_avatar_db_fail' => 'Failed to save to database: ',
        'err_avatar_upload_fail' => 'Failed to upload file to server.',
        'err_avatar_select' => 'Please select an image file to upload.',
        'err_pass_required' => 'All password fields are required.',
        'err_pass_mismatch' => 'New password confirmation does not match.',
        'err_pass_length' => 'New password must be at least 6 characters.',
        'msg_pass_success' => 'Password updated successfully.',
        'err_pass_fail' => 'Failed to change password: ',
        'err_pass_invalid' => 'Current password is invalid.',
        'msg_prefs_success' => 'System preferences saved successfully.',
        'err_prefs_fail' => 'Failed to update preferences: ',
        
        // Notifikasi Archive
        'notif_archive_title' => 'Notification Center',
        'notif_all' => 'All Notifications',
        'btn_clear_all' => 'Clear All',
        'lbl_view_detail' => 'View Detail',
        'empty_notif_archive' => 'No notifications received yet.',
        'confirm_clear_all' => 'Are you sure you want to clear all notification history?'
    ]
];

// Translation helper function
if (!function_exists('__')) {
    function __($key) {
        global $dict, $lang_code;
        return isset($dict[$lang_code][$key]) ? $dict[$lang_code][$key] : (isset($dict['id'][$key]) ? $dict['id'][$key] : $key);
    }
}
